...
class CatalogCfo(Base):
    __tablename__ = 'rs1c_md_catalog_cfo'
...
class CatalogPodr(Base):
    __tablename__ = 'rs1c_md_catalog_podr'
    __table_args__ = {'schema': 'rsrc'}
...
class CatalogStOb(Base):
    __tablename__ = 'rs1c_md_catalog_st_ob'
    __table_args__ = {'schema': 'rsrc'}
...
class CatalogTypeRsp(Base):
    __tablename__ = 'rs1c_md_catalog_type_rsp'
    __table_args__ = {'schema': 'rsrc'}
...
class CatalogNom(Base):
    __tablename__ = 'rs1c_md_catalog_n'
    __table_args__ = {'schema': 'rsrc'}
...
class CatalogNomGroups(Base):
    __tablename__ = 'rs1c_md_catalog_ng'
    __table_args__ = {'schema': 'rsrc'}

    НоменклатурнаяГруппа = Column(Text, primary_key=True)
    ЭтоГруппа = Column(Boolean)
    Наименование = Column(Text, primary_key=True)
    ТипСтатьи = Column(Text, nullable=True)
    Кодификатор = Column(Text, nullable=True)
    Родитель = Column(Text, primary_key=True)
    ПометкаУдаления = Column(Boolean)


class CatalogScenariy(Base):
    __tablename__ = 'rs1c_md_catalog_scenariy'
    __table_args__ = {'schema': 'rsrc'}
...
class Documents(Base):
    __tablename__ = 'rs1c_md_documents'
    __table_args__ = {'schema': 'rsrc'}
